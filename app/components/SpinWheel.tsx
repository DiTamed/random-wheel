...
const PRIZES = [
  ...
  { name: 'May Mắn',   color: '#C77DFF', textColor: '#fff' },
];
...
const CONFETTI_COLORS = ['#FF6B6B','#FFD93D','#6BCB77','#4D96FF','#C77DFF','#FF9F43','#FF6EFF','#fff'];
...
export default function SpinWheel() {
  ...
  const [result,   setResult]   = useState<{ prize: string } | null>(null);
  ...
  const validate = () => {
    ...
    if (!/^(0|\+84)[0-9]{8,10}$/.test(phone.trim())) return 'Số điện thoại không hợp lệ!';
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      const res  = await fetch('/api/spin', {
        ...
        body: JSON.stringify({ name: name.trim(), phone: phone.trim(), branch }),
      });
      const data = await res.json();

      if (res.status === 409) {
        setLoading(false);
        setError(data.error || 'Số điện thoại này đã tham gia rồi!');
        return;
      }
      ...
      const curNorm    = ((rotRef.current  % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);
      ...
      const animate = (now: number) => {
        ...
        if (t < 1) {
          ...
          setSpinning(false);
          setResult({ prize: PRIZES[wi].name });
          setModal(true);
          setConfetti(true);
    ...
    <>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden>
        ...
        <div style={{
          ...
          background:'radial-gradient(circle, rgba(29,78,216,0.18), transparent)',
          borderRadius:'50%', bottom:-120, right:-100,
          animation:'float 11s ease-in-out infinite reverse',
        }}/>
      </div>

      {/* ── Confetti ── */}
      {confetti && CONFETTI_ITEMS.map(c => (
        <div key={c.id} className="confetti-piece" style={{
          left: c.left, width: c.size, height: c.size,
          backgroundColor: c.color, borderRadius: c.borderRadius,
      ...
      <main style={{
        ...
        padding:'32px 16px', gap:32, position:'relative',
      }}>
        {/* Header */}
        <header className="animate-slide-up" style={{ textAlign:'center' }}>
          <div style={{ fontSize:13, letterSpacing:3, color:'rgba(255,215,0,0.7)', textTransform:'uppercase', marginBottom:8 }}>
            Chương Trình Khuyến Mãi
          </div>
          <h1 style={{
            fontSize:'clamp(28px,5vw,48px)', fontWeight:900, lineHeight:1.1,
        ...
        <div style={{
          ...
          alignItems:'center', justifyContent:'center', width:'100%', maxWidth:900,
        }}>
          {/* Wheel */}
          <div className="animate-slide-up delay-100" style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:20 }}>
            <div className="wheel-halo animate-float" style={{ borderRadius:'50%' }}>
              <canvas
                ref={canvasRef}
                width={380}
                height={410}
          ...
          {/* Form */}
          <div className="glass animate-slide-up delay-200" style={{
            padding:32, width:'100%', maxWidth:380,
          }}>
            <h2 style={{ fontSize:20, fontWeight:700, marginBottom:24, color:'#FFD700' }}>
              Thông Tin Đăng Ký
            </h2>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
              {/* Name */}
              <div>
                <label className="form-label" style={{ display:'block', fontSize:12, fontWeight:600, letterSpacing:'0.5px', color:'rgba(255,255,255,0.6)', textTransform:'uppercase', marginBottom:7 }}>
                  Họ và Tên *
                </label>
                <input
                  id="field-name"
              ...
              {error && (
                <div style={{
                  ...
                  color:'#ff8888', borderRadius:10, padding:'10px 14px', fontSize:14,
                }}>
                  {error}
                </div>
              )}
            ...
            <div style={{ marginTop:24, borderTop:'1px solid rgba(255,255,255,0.08)', paddingTop:20 }}>
              <p style={{ fontSize:12, color:'rgba(255,255,255,0.45)', marginBottom:12, letterSpacing:'0.5px', textTransform:'uppercase' }}>
                Phần thưởng
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {PRIZES.map(p => (
                  <div key={p.name} style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <div style={{ width:14, height:14, borderRadius:3, backgroundColor:p.color, flexShrink:0 }}/>
      ...
      {modal && result && (
        <div className="modal-overlay" onClick={handleClose}>
          <div
            ...
            style={{ padding:'48px 40px', textAlign:'center', maxWidth:420, width:'90%', position:'relative' }}
          >
            {/* Close */}
            <button
              onClick={handleClose}
            ...
            <div style={{ fontSize:13, letterSpacing:2, color:'rgba(255,215,0,0.7)', textTransform:'uppercase', marginBottom:8 }}>
              Chúc Mừng!
            </div>

            <h2 style={{ fontSize:32, fontWeight:900, marginBottom:6,
              background:'linear-gradient(135deg,#FFD700,#FF8C00)', WebkitBackgroundClip:'text',
              WebkitTextFillColor:'transparent', backgroundClip:'text' }}>
              {result.prize}
            </h2>

            <p style={{ color:'rgba(255,255,255,0.55)', fontSize:15, lineHeight:1.6, marginTop:12 }}>
              Bạn đã nhận được phần thưởng <strong style={{color:'#FFD700'}}>{result.prize}</strong>.<br/>
              Vui lòng liên hệ nhân viên chi nhánh để nhận thưởng.
            </p>

            {/* Stars */}
            <div style={{ marginTop:20, color:'#FFD700', fontSize:26, letterSpacing:4 }}>
              ★ ★ ★ ★ ★
            </div>
